...
const getMonthLabel = (key) => {
  const [y, m] = key.split("-");
  return new Date(parseInt(y), parseInt(m) - 1, 1).toLocaleString("en-AU", { month: "long", year: "numeric" });
};
...
export default function BudgetTracker() {
  ...
  const removePayment = (itemId, payId) => {
    setItems(prev => prev.map(item =>
      item.id === itemId
        ? { ...item, payments: item.payments.filter(p => p.id !== payId) }
        : item
    ));
  };

  const switchMonth = (key) => {
    // If switching to a future/new month, roll over from latest existing
    setAllMonths(prev => {
      if (!prev[key]) {
  ...
  // Generate list of months (past 3 + current + next 2)
  const monthOptions = useMemo(() => {
    ...
    // Also include any stored months not in range
              ...
              {catItems.map((item, idx) => {
                ...
                return (
                  <div key={item.id} style={{ padding: "10px 14px", background: item.paid ? (isIncome ? "rgba(74,222,128,0.07)" : "rgba(74,222,128,0.04)") : "transparent", borderTop: idx > 0 ? "1px solid #1a1f2e" : "none", opacity: item.paid ? 0.7 : 1, transition: "opacity 0.2s" }}>
                    {/* Row 1: Checkbox + Label + Paid badge */}
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      ...
                      {item.paid && <span style={{ fontSize: 10, fontWeight: 700, color: "#4ade80", background: "#052e16", padding: "2px 8px", borderRadius: 5, letterSpacing: "0.05em", whiteSpace: "nowrap" }}>{isIncome ? "RECV'D" : "PAID"}</span>}
                    </div>
                    {/* Row 2: Amount + Frequency + Date */}
